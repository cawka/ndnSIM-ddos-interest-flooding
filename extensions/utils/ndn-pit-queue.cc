...
#include "ns3/ndnSIM/utils/ndn-limits-window.h"
                   ...
                   double updatedWeight)
{
  ...
  if (queue == m_queues.end ())
    {
      pair<PerInFaceQueue::iterator, bool> itemPair =
        m_queues.insert (make_pair (inFace, boost::make_shared< WeightedQueue > (WeightedQueue (Queue (), updatedWeight, 0))));
  ...
  Ptr<Limits> limits = inFace->GetObject<Limits> ();
  if (limits == 0)
    {
      NS_FATAL_ERROR ("PIT queue requires that per-face limits are enabled");
    }
  
  if ((!limits->IsEnabled () && queue->second->get<0> ().size () >= m_maxQueueSize) ||
      (limits->IsEnabled () && queue->second->get<0> ().size () >= 0.8*(limits->GetCurrentLimitRate () * limits->GetMaxDelay ())))
    {
      return false;
    }
